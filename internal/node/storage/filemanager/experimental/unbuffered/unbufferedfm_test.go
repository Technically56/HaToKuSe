...
func TestWriteAndReadIntegrity(t *testing.T) {
	...
	// Note: explicitly casting 100 to int32 to match NewFileManager signature if strict
...
func TestDynamicSharding(t *testing.T) {
	...
	// Use tiny shard count (2) to easily check folder existence
	shardCount := uint32(2)
	...
	if !found {
		t.Errorf("File not found in expected shards (0 or 1)")
	}
}
...
func TestConcurrentReadWrite(t *testing.T) {
	...
	// Larger lock pool to reduce contention
		...
		go func(id int) {
			...
			for j := 0; j < filesPerWorker; j++ {
				...
				if string(readVal) != content {
					t.Errorf("Data corruption for %s", fileName)
				}
			}
		}(i)
	}
...
// Since buffering is off, this measures OS Page Cache write speed.
